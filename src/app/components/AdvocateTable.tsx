...
const AdvocateTable = ({ headers, rows }: AdvocateTableProps) => {
  return (
        ...
        <thead>
          <tr>
            {headers.map((headText, i) => (
              <th
                key={`th-${i}-${headText}`}
                className={`${TABLE_CLASSNAME.th}`}
              >
                <p className={`${TABLE_CLASSNAME.thp}`}>{headText}</p>
              </th>
            ))}
          </tr>
        </thead>
